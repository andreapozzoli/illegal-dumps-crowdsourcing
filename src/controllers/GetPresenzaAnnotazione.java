...
@WebServlet("/GetPresenzaAnnotazione")
public class GetPresenzaAnnotazione extends HttpServlet {
...
boolean presente = true;
		...
		try {
			presente=aDAO.presenzaAnnotazione(idLavoratore, idImmagine);
			
		} catch (SQLException e) {
			ris= "erroredati";
		}
		writeResponse(response, presente, ris);
		}
	...
	private void writeResponse(HttpServletResponse response,boolean presente, String ris) throws IOException {
		if (ris.equals("")) {
		if(presente)
		response.getWriter().write("presente");
		else
			response.getWriter().write("assente");
		}
		else
